/* api_request.js
 *
 * Provides authentication with the TinEye API.
 *
...
ApiRequest.prototype.generateNonce = function(nonceLength) {
  ...
  */
  ...
  nonceLength = nonceLength || this.minNonceLength;

  if (
    isNaN(parseInt(nonceLength)) ||
    (nonceLength < this.minNonceLength || nonceLength > this.maxNonceLength)
  ) {
    throw new RangeError(
      "Nonce length must be an int between " +
        this.minNonceLength +
        " and " +
        this.maxNonceLength
    );
  ...
  for (var i = 0; i < nonceLength; i++) {
    nonce += this.nonceAllowableChars.charAt(
      Math.floor(Math.random() * this.nonceAllowableChars.length)
    );
  }
...
ApiRequest.prototype.generateGetHmacSignature = function(
  method,
  nonce,
  date,
  requestParams
  ...
  */
  ...
  var toSign =
    this.privateKey + httpVerb + String(date) + nonce + requestUrl + paramStr;
...
ApiRequest.prototype.sortParams = function(requestParams, lowercase) {
  /*
    ...
    - `lowercase`, whether to lowercase the image URL.
  ...
  */
  ...
  for (var i = 0; i < keys.length; i++) {
    sortedPairs.push(
      keys[i] + "=" + unsortedParams[String(keys[i].toLowerCase())]
    );
  }
...
ApiRequest.prototype.requestUrl = function(
  method,
  nonce,
  date,
  apiSignature,
  ...
  */
  ...
  if (extraParams !== "") requestUrl += "&" + extraParams;
...
ApiRequest.prototype.getRequest = function(method, requestParams) {
  ...
  */
  ...
  var apiSignature = this.generateGetHmacSignature(
    method,
    nonce,
    date,
    requestParams
...
ApiRequest.prototype.postRequest = function(
  method,
  filename,
  requestParams,
  boundary
  ...
  /*
    ...
    Returns: a URL to send the search to.
  */

  if (
    filename === null ||
    String(filename).replace(/^\s+|\s+$/g, "").length === 0
  ) {
    throw new Error("400", "Must specify an image to search for.");
  }

  var nonce = this.generateNonce();
  var date = parseInt(new Date().getTime() / 1000);
  var apiSignature = this.generatePostHmacSignature(
    "search",
    boundary,
    nonce,
    date,
